...
const Sidebar = ({ 
  ...
  const menuItems = [
    ...
    { id: "assignments", label: "Tugas", icon: faTasks, badge: { count: 4, color: "red" } },
    { id: "announcements", label: "Pengumuman", icon: faComment, badge: { count: 3, color: "blue" } },
  ];
        ...
        <nav className="flex-1 px-4 py-6 space-y-2">
          {menuItems.map((item) => (
            ...
            >
              ...
              {item.badge && (
                <span className={`ml-auto bg-${item.badge.color}-100 text-${item.badge.color}-800 text-xs font-bold px-2 py-1 rounded-full`}>
                  {item.badge.count}
                </span>
              )}
            </button>
